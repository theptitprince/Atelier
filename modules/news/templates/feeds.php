                    ...
                    <tr class="<?= (int) $feed['is_active'] === 1 ? '' : 'is-disabled' ?><?= $feed['last_status'] === 'error' ? ' is-warning' : '' ?>">
                        ...
                        <td class="col-actions">
                            <span class="table-actions">
                                ...
                                <?php if ($rights['delete']): ?><button type="button" class="btn btn--sm btn--icon btn--ghost" data-action="feed-delete" data-params='{"id":<?= (int) $feed['id'] ?>}' data-confirm="Retirer « <?= $e($feed['title']) ?> » ? Ses entrées non archivées seront supprimées ; les faits archivés sont conservés." data-danger title="Retirer"><svg class="icon" aria-hidden="true"><use href="#i-trash"></use></svg></button><?php endif; ?>
                            </span>
                        </td>
                    </tr>
                <?php endforeach; ?>
