...
- **Corbeille et registre commun** (1.2.4) : la mise à la corbeille d'un fait archivé (ou d'un flux) est signalée au registre (`registry->trash`), la restauration l'en retire (`registry->restore`) ; un fait archivé en corbeille n'apparaît plus sous ses tags, dans l'Explorateur ni dans les éléments liés des autres modules. Pas de cascade depuis un flux : ses faits archivés restent consultables, donc visibles, et ses entrées non archivées ne sont jamais inscrites au registre. La fiche d'un fait archivé n'affiche plus les relations vers des informations en corbeille. La migration `004_registry_trash` a marqué les éléments déjà en corbeille.
