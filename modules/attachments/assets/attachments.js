 ...
 *  - téléversement fichier par fichier (chaque refus est rapporté individuellement) ;
      ...
      ctx.on(ctx.root, 'submit', 'form[data-attachments-upload]', async (e, form) => {
        ...
        try {
          for (let i = 0; i < pending.length; i++) {
            ...
            body.append('info_id', form.querySelector('[name="info_id"]').value);
            try {
              ...
              if (err && err.kind === 'validation' && err.fields && (err.fields.info_id || err.fields.description)) { Atelier.forms.showErrors(form, err.fields); break; }
            }
            renderQueue(ctx.root);
          }
        } finally {
