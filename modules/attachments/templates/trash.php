...
<div class="module module-attachments">
    <?php if ($rows === []): ?>
        <?= $module->renderCore('state', ['type' => 'empty', 'title' => 'La corbeille est vide', 'message' => 'Les fichiers supprimés y restent ' . $retentionDays . ' jours avant purge automatique (fichier et métadonnées).']) ?>
    <?php else: ?>
            ...
            <table class="table attachments__table">
                ...
                <?php foreach ($rows as $row): ?>
                    <tr>
                        ...
                        <td><a href="#" data-route="show/<?= $e($row['id']) ?>"><?= $e($row['original_name']) ?></a></td>
                        ...
                        <td class="col-actions">
                            <span class="table-actions">
                                ...
                                <button type="button" class="btn btn--sm btn--outline-danger" data-action="purge" data-params='{"id":"<?= $e($row['id']) ?>"}' data-confirm="Supprimer définitivement « <?= $e($row['original_name']) ?> » ? Cette action est irréversible." data-danger><svg class="icon" aria-hidden="true"><use href="#i-trash"></use></svg> Supprimer</button>
                            </span>
                        </td>
                    </tr>
                <?php endforeach; ?>
