...
Suivi de l’entretien des biens du foyer : voiture, chaudière, électroménager, habitation, jardin, matériel informatique. Version 1.2.0.
...
- **Historique** des interventions : date, compteur, coût, intervenant, notes, documents joints ; filtres par équipement, année et texte ; total des coûts ; export CSV. Chaque ligne porte un dépliant « documents » (liste, téléchargement `/files/{id}`, dépôt direct).
...
Tables `maintenance_asset`, `maintenance_job`, `maintenance_log`, chacune avec une colonne `deleted_at` (corbeille ; migration `002_soft_delete` pour les tâches et interventions). Les échéances et dates de réalisation sont des jours calendaires stockés en texte `AAAA-MM-JJ` ; les montants sont en centimes.
