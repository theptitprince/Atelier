...
- **Registre commun** (1.2.4) : la mise à la corbeille signale la page au registre (`registry->trash`), la restauration l'en retire (`registry->restore`). Une page en corbeille n'apparaît donc plus sous ses tags, dans l'Explorateur, ni dans les éléments liés des autres modules (fiche d'un projet, d'un point GPS) ; ses tags et relations sont conservés et réapparaissent à la restauration. La migration `002_registry_trash` a marqué les pages déjà en corbeille.
